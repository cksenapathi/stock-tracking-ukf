...
def main():
    ...
    for i in range(len(y_test)):
        ...
        new_price_sigma = price_ukf.state_transition(wt_ukf.mean, price_sigma, price_mean_wts, price_cov_wts)[0]
        ...
        output_price_sigma, output_price_mean, output_price_cov = price_ukf.output(input_, new_price_sigma, price_mean_wts, price_covar_wts, np.mean(err_data), np.cov(err_data))
        ...
        output_grad_sigma, output_grad_mean, output_grad_cov = wt_ukf.output_state(new_param_sigma, param_mean_wts, param_cov_wts, shapes, input_, output_price_mean)
        # Getting the real value that is used to update both
        measured_price = y_test[i, 0]

        # Updating the price ukf
        wts, _ = price_ukf.update(measured_price, price_sigma, price_mean_wts, price_cov_wts, output_price_sigma, output_price_mean, output_price_cov)
